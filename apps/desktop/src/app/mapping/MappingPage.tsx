...
export function MappingPage({
  ...
  instrumentLibrary: import("@midi-playground/core").InstrumentDef[];
}) {
  ...
  // Lookup profile from library (dynamic) or fallback to legacy ID match
  const targetProfile = targetDevice?.instrumentId
    ? (instrumentLibrary || []).find(
        (p) =>
          p.meta.id === targetDevice.instrumentId ||
          // Fallback: if no ID in meta (new format), try to construct one?
          // Actually, simplistic matching on legacy ID is key here.
          // New profiles generated by ingestion won't have 'id', they have vendor/model.
          // But 'targetDevice.instrumentId' comes from where?
          // If it's a legacy project, it's 'microfreak'.
          // If it's a new project, we might store 'vendor:model'.
          // For now, let's assume legacy ID priority if present.
          p.meta.id === targetDevice.instrumentId
      ) ?? null
    : null;

  // Adapter for AssignmentWizard / UI which expects the old shape?
  // Old shape: { cc: [{id, label, cc}] }
  // New shape: { parameters: [{id, label, msg: {index: cc}}] }
  // We need a helper to adapt or update the UI to process InstrumentDef.
  const adaptedProfile = targetProfile
    ? {
        ...targetProfile,
        cc: targetProfile.parameters
  ...
  // Use adapted profile for compatibility with existing UI logic below
  const activeProfile = adaptedProfile;
  const macroTargets = [
    {
      label: "Filter",
