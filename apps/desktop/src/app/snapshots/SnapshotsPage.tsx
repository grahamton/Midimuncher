...
import type { SnapshotClockSource, SnapshotQueueStatus } from "../../../shared/ipcTypes";
import type { SnapshotMode, SnapshotQuantize, SnapshotsState } from "../../../shared/projectTypes";
...
export function parseSnapshotId(snapshotId: string): { bankId: string; slotId: string } | null {
  const idx = snapshotId.indexOf(":");
  if (idx <= 0) return null;
  return { bankId: snapshotId.slice(0, idx), slotId: snapshotId.slice(idx + 1) };
}

export function findSnapshotSlot(
  snapshotId: string,
  state: SnapshotsState
): { bank: SnapshotsState["banks"][number]; slot: SnapshotsState["banks"][number]["slots"][number] } | null {
...
export function findSnapshotIdByName(name: string, state: SnapshotsState): string | null {
...
export function listSnapshotNames(state: SnapshotsState): string[] {
  const bank = state.banks.find((b) => b.id === state.activeBankId) ?? state.banks[0];
  return (bank?.slots ?? []).map((s) => s.name);
}

export function writeSnapshotToSlot(state: SnapshotsState, snapshotId: string, snapshot: SnapshotState): SnapshotsState {
  const parsed = parseSnapshotId(snapshotId);
...
export function SnapshotsPage({
  ...
  const activeBank =
    snapshots.banks.find((b) => b.id === snapshots.activeBankId) ?? snapshots.banks[0];
  const activeSlot = activeSnapshotId ? findSnapshotSlot(activeSnapshotId, snapshots) : null;
  const pendingSlot = pendingSnapshotId ? findSnapshotSlot(pendingSnapshotId, snapshots) : null;
  ...
  return (
    <Page>
        ...
        right={
          <div style={styles.row}>
            ...
            <input
              ...
              onChange={(e) => onChangeSnapshotCommitDelay(Number(e.target.value) || 0)}
            ...
            <input
              ...
              onChange={(e) =>
                onChangeSnapshotCycleBars(Math.min(Math.max(Number(e.target.value) || 1, 1), 32))
              }
            ...
            <select
              ...
              onChange={(e) => onChangeSnapshotClockSource(e.target.value as SnapshotClockSource)}
            >
            ...
            <select
              style={styles.select}
              value={snapshotMode}
              onChange={(e) => onChangeSnapshotMode(e.target.value as SnapshotMode)}
            >
            ...
            <button style={styles.btnPrimary} onClick={() => activeSnapshotId && onSelectSnapshot(activeSnapshotId)}>
              Send Snapshot
            </button>
            <button style={styles.btnSecondary} onClick={() => activeSnapshotId && onCapture(activeSnapshotId)} disabled={!activeSnapshotId}>
              Capture
            </button>
            {queueLength > 0 ? (
        ...
        <Panel title="Snapshot Pads">
          ...
          >
            {(activeBank?.slots ?? []).map((slot, idx) => {
              const id = activeBank ? makeSnapshotId(activeBank.id, slot.id) : slot.id;
        ...
        <Panel title={`Selected: ${activeSlot?.slot.name ?? "None"}`}>
          <div style={styles.card}>
            <div style={styles.row}>
              ...
              {pendingSlot ? (
                <span style={styles.muted}>Pending: {pendingSlot.slot.name}</span>
              ) : null}
            </div>
            <div style={styles.row}>
      ...
      <div style={styles.queueStrip}>
        ...
        <span style={styles.pill}>{pendingSlot ? `Pending: ${pendingSlot.slot.name}` : "Idle"}</span>
        {pendingSlot ? (
